...
class ReconnectionManager {
  ...
  final StreamController<ReconnectionState> _reconnectionStateStreamController =
      StreamController.broadcast();

  Stream<ReconnectionState> get reconnectionStateStream =>
      _reconnectionStateStreamController.stream;
              ...
              if (onError is ResponseException) {
                loginWithToken(accessToken, refreshToken).then((_) {
                  ...
                  if (ex.status == statusTokenExpired) {
                    _reconnectionStateStreamController
                        .add(ReconnectionState.tokenExpired);
                  }
                });
              } else {
                log('[ReconnectionManager]',
...
enum ReconnectionState { idle, tokenExpired }
